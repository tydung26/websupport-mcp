...
- [x] `src/tools/v1/common.ts` — `userIdArg` defaulting to the literal `self`; declare paging args only where verified against live responses (v1 pagination is documented on `GET /v1/user` and `/invoice`; do not assume uniformity) — **done**: paging is declared per endpoint on measured evidence, not assumed. `/service` ignores `pagesize` (echoes `null`); `/zone`, `/hosting`, `/vps`, `/invoice` all honour it, so `ws_service_list` advertises no paging at all
- [x] `user.ts`, `service.ts`, `zone.ts` first — validate the pattern before scaling out — **done** as `account-tools.ts` (one module for the three account-level groups rather than three near-empty files)
- [x] `hosting.ts` — hosting list/get, vhost list/get, `ws_hosting_stats` (`kind` → `size-stats|domain-stats|ftp-stats`) — **done** as `hosting-tools.ts`
- [x] `database.ts` — db list/get, dbusers list, `ws_db_stats` (`kind` → `size-stats|cpu-stats`) — **done** as `database-tools.ts`
- [x] `mailbox.ts` — mailbox list/get, `ws_mail_stats` (optional `domain`) — **done** as `mailbox-tools.ts`
- [x] `vps.ts` — vps list/get, `ws_vps_stats` (`kind` → `cpu-stats|traffic-stats`), `ws_vps_vnc` (returns sensitive access data — flag in the description, never log the body) — **done** as `vps-tools.ts`
- [x] `invoice.ts` — list/get, `ws_invoice_pdf` with `format: 'base64' | 'binary'`; `binary` returns metadata + byte count, never inlined bytes — **done** as `invoice-tools.ts`; `binary` is the default so a document is never dumped into a conversation unasked, and the transport gained a `responseType: 'bytes'` mode so PDF bytes are not corrupted by a text decode
- [x] Suffix-only variants collapse behind a `kind` enum with one lookup object per tool (DRY) — **done** via a shared `suffixEnum` helper
- [x] Register all 24; confirm every one stays visible with zero env opt-ins — **done and verified over a real MCP handshake**: 29 tools with no opt-ins (5 v2 read + 24 v1 read), 35 with `ALLOW_WRITE`, 31 with `ALLOW_DESTRUCTIVE`, 37 with both
- [~] Live-verify each tool; record doc-vs-reality field discrepancies under Findings below — **all 24 exercised live through their real handlers**: 6 return parsed results, 18 reach the resource layer and fail on the record rather than the route. Full data-bearing verification still needs an account that owns something
...
- [x] `src/tools/confirm.ts` — one shared `confirm: z.literal(true)` fragment + wording, so the gate is identical everywhere — **done** (landed early, since two v2 tools already needed it)
- [x] Registry-wide invariant test: _every_ tool of tier `destructive` includes the confirm fragment — enforced structurally, not by author discipline — **done**: also asserts the converse, that no non-destructive tool declares one, and that all seven share identical wording
- [x] `database-write.ts` — `ws_db_create` (write), `ws_db_update` (write), `ws_db_delete` (destructive) — **done** as `database-write-tools.ts`
- [x] `mailbox-write.ts` — create/update/delete; mirror both documented quirks: update is `POST` (not `PUT`), and write paths are rooted at `.../hosting/:hid/domain/:did/mailbox` while Phase 4 reads are rooted at `.../hosting/:hid/mailbox`. Do not normalise — **done** as `mailbox-write-tools.ts`; both quirks confirmed live, and the symmetrical-looking `POST .../hosting/:hid/mailbox` proved to be no route at all
- [x] `vps-write.ts` — `ws_vps_reboot` (write), `ws_vps_hard_reboot` (destructive), snapshot list (read) / create (write) / restore (destructive) / delete (destructive) — **done** as `vps-write-tools.ts`
- [x] `service-write.ts` — `ws_service_set_auto_extend`, sending only `autoExtend`, no arbitrary service properties — **done**; a test asserts arbitrary service properties are rejected, so billing terms cannot be edited through it
- [x] Destructive descriptions name the concrete irreversible effect, e.g. restore: "Overwrites the VPS disk with the named snapshot. All data written since that snapshot is permanently lost." — **done**, enforced by a registry test
- [x] Offline tests — each destructive tool rejects a call without `confirm: true`; write tools absent without `WEBSUPPORT_ALLOW_WRITE=1` — **done** for all seven, including `confirm: false`
- [ ] Live-verify on disposable resources only: scratch database and scratch mailbox create + delete — **blocked**: the available account owns no hosting service, so there is nowhere to create a scratch database or mailbox
- [x] `ws_vps_hard_reboot` and `ws_vps_snapshot_restore` verified by request construction (fetch spy asserting exact method + path) — **not** live-fired; record the limitation — **done and recorded as the complete verification for those two**; they are never live-fired
- [x] Test greps the registry for `/order` and `/pay` paths and asserts none exist — **done**: greps the source tree, not just tool metadata, so a path literal cannot slip in through a handler
...
- [x] `package.json` — `bin`, `files` whitelist (build output + `assets/` only), `prepublishOnly`, `engines`; `npm pack --dry-run` shows no `.env`, no test files, no local paths — **done**: the build bundles to a single `dist/index.js` with the OpenAPI spec inlined, so the tarball is 4 files and ships no sources, tests, assets or env files. A CI gate asserts exactly one entrypoint and rejects the rest
- [x] `.github/workflows/ci.yml` — typecheck, lint, offline tests, build, bundle smoke test and package gate on node **22 and 24**; `spec-drift` isolated to a separate non-blocking job so a Websupport outage cannot red the build — **done**. Node 20 dropped: it reached end of life 2026-04-30, and the bundler's own floor is ^22.18 || >=24.11
...
all three env configs · CI green on node 22 and 24 with network tests non-blocking · verification
...
| 2026-08-25 | 4     | **All 30 planned v1 read routes exist.** Validated live on an account owning nothing, by exploiting that the API distinguishes a missing *route* (`404 The system is unable to find the requested action "…"`) from a missing *record* (`404 Hosting not found`). Two deliberate controls — a nonsense path and `/v1/user/self/mailbox` — both classified as missing routes, so the discriminator is sound. No path in the plan needed correcting. |
| 2026-08-25 | 4     | **Correction to the 2026-08-25 mailbox finding.** The earlier entry implied the plan had mailbox reads rooted at `/v1/user/:id/mailbox`. It did not — the plan always specified `/v1/user/:id/hosting/:hid/mailbox`, which is a valid route. The 404 came from a path invented during probing, not from a plan error. The statement "mailbox reads are not user-rooted" is true of the API; it was not a defect in the plan. |
| 2026-08-25 | 4     | **v1 paging is not uniform, confirmed by measurement.** Sending `pagesize=7` and reading back `pager.pagesize`: `/service` answers `null` (parameter ignored), while `/zone`, `/hosting`, `/vps` and `/invoice` all echo `7`. `ws_service_list` therefore declares no paging arguments. Whether `page` is honoured remains unproven — an empty account clamps it, so "ignored" and "clamped" are indistinguishable until there is more than one page of records. |
| 2026-08-25 | 2, 4  | `requestJson` gained `responseType: 'bytes'`. Without it a PDF response would go through `response.text()` and be silently corrupted. The transport returns a `Uint8Array`; the tool layer decides whether to base64-encode it or report metadata only. Error bodies are always parsed normally, never as bytes. |
| 2026-08-25 | 5     | **All 13 v1 mutating routes exist.** Validated live using ids for resources that do not exist, so every call could only reach a 404 at the resource layer — nothing was created, altered or destroyed. Three controls classified correctly as missing routes. This technique is safe only against ids known not to exist; it must not be repeated on a populated account. |
| 2026-08-25 | 5     | **The mailbox read/write asymmetry is real, not a documentation error.** Writes live at `.../hosting/:hid/domain/:did/mailbox`; the symmetrical `POST .../hosting/:hid/mailbox` returns `404 The system is unable to find the requested action`, i.e. it is not a route. Update is `POST`, not `PUT`. Both mirrored rather than normalised, as the plan required. |
| 2026-08-25 | 5     | **v1 publishes no machine-readable spec**, confirmed: `/v1/docs/openapi.json`, `/v1/docs`, `/v1/openapi.json`, `/docs/v1/openapi.json` and `/v1/doc` all 404, while `/v2/docs` returns 200. The database and mailbox create/update field sets are therefore unverified — a POST to a nonexistent hosting id is rejected before body validation runs, so they cannot be discovered live either. Those four schemas require only what a record cannot exist without and forward any other field to the API, whose validation error is surfaced verbatim. This is a deliberate, documented deviation from the `z.strictObject` rule, confined to `write-body.ts`, to be tightened once the field sets are confirmed. |
| 2026-08-25 | 5     | Full 50-tool surface verified over a real MCP stdio handshake. Tool counts by env: none **30**, `ALLOW_WRITE` **43**, `ALLOW_DESTRUCTIVE` **37**, both **50**. Tier split 30 read / 13 write / 7 destructive. |
| 2026-08-25 | 6     | **Build switched from `tsc` to a bundler (`tsdown`, Rolldown).** The server is launched as a bin through `npx`, so startup cost is module resolution rather than compute. Output went from 60 emitted files to a single 96 kB `dist/index.js`; the tarball is 4 files / 56 kB. The vendored OpenAPI spec is imported rather than read from disk, so the bundler inlines it — a `readFileSync` against `import.meta.url` resolves differently once the output collapses to one file, and would have broken silently. |
| 2026-08-25 | 6     | Two tooling traps worth recording. `tsdown` emits `.mjs` by default; with `"type": "module"` and `bin` pointing at `dist/index.js`, that would have published a package whose entrypoint did not exist — pinned via `outExtensions`. And npm 10 cannot resolve tsdown's dependency tree at all, failing with `Cannot read properties of null (reading 'edgesOut')` rather than an engine error; npm >= 11 is required to build. |
| 2026-08-25 | 6     | **Node 20 dropped.** It reached end of life 2026-04-30 (confirmed against endoflife.date), and the bundler's floor is ^22.18 || >=24.11 regardless. `engines` is now `>=22`; CI covers the two live LTS lines, 22 and 24, and builds and smoke-tests the bundle on each. Because both runtimes can build, the separate build job folded back into the matrix. |
| 2026-08-25 | 6     | CI now smoke-tests the built artifact over a real stdio handshake and asserts the tool count, so a bundle that compiles but cannot start fails the build. That check is what would have caught the `.mjs` entrypoint mistake automatically. |
...
| 2026-08-25 | **v1 read surface implemented — 37 tools registered.** All 24 v1 read tools written across six modules (`account`, `hosting`, `database`, `mailbox`, `vps`, `invoice`) plus a shared `common.ts`. Rather than build from HTML docs and hope, every route was validated live first: the API distinguishes a missing route from a missing record, so all 30 route shapes were confirmed against an account owning nothing — and none of the planned paths needed correcting. Paging was then measured rather than assumed, which found `/service` silently ignoring `pagesize` where four sibling endpoints honour it; `ws_service_list` consequently advertises none. The transport gained a `bytes` response mode so invoice PDFs are not corrupted by a text decode, and `ws_invoice_pdf` defaults to reporting metadata rather than dumping a document into the conversation. `ws_user_get` and `ws_vps_vnc` carry explicit sensitivity warnings in their descriptions. Verified over a real MCP handshake at 29 / 35 / 31 / 37 tools for the four env permutations. 234 offline tests green; typecheck, lint and build clean. An earlier finding about mailbox rooting is corrected above: it was a probing mistake, not a plan defect. |
| 2026-08-25 | **v1 mutating surface implemented — the full 50-tool registry is now live.** Thirteen tools across database, mailbox, VPS and service writes. Routes were validated against the live API first, using ids for resources that do not exist so nothing could be mutated; all thirteen exist, and the symmetrical-looking mailbox write path proved not to be a route at all, confirming the documented read/write asymmetry is genuine. Tiering follows what an operation destroys rather than whether it writes: graceful reboot is `write` while a hard power-cycle is `destructive`, snapshot create is `write` while restore is `destructive`. `ws_vps_snapshot_list` stays tier `read` so a read-only deployment can still see recovery points. `ws_vps_hard_reboot` and `ws_vps_snapshot_restore` are verified by request construction only and are never live-fired — that limitation is recorded rather than glossed. One honest gap: v1 has no published spec and the database/mailbox create field sets could not be observed, so those four schemas forward unknown fields instead of pretending to a contract that was never verified. 261 offline tests green; handshake verified at 30 / 43 / 37 / 50. |
| 2026-08-25 | **Packaging modernised; Node 20 support removed.** The build moved from `tsc` emitting 60 files to `tsdown` producing one 96 kB bundle with the OpenAPI spec inlined, cutting the tarball to 4 files and removing every runtime disk read. Node 20 was dropped outright — it reached end of life four months ago and the bundler will not run on it — so `engines` is now `>=22` and CI covers the live LTS lines 22 and 24. Since both can build, the build job merged back into the test matrix, and the bundle is now smoke-tested over a real stdio handshake on each runtime. Branch protection requires the matrix. Two traps recorded in Findings: tsdown's default `.mjs` extension would have shipped a missing entrypoint, and npm 10 cannot install the bundler at all. |
